...
game_sc = Canvas(tk, width=W*TILE+1, height=H*TILE+1, bg="pink", highlightthickness=0)
game_sc.place(x=50, y=100, anchor=NW)

img_obj1 = PhotoImage(file="imgs/bg.png")
sc.create_image(0, 0, anchor = NW, image = img_obj1)
img_obj2 = PhotoImage(file="imgs/bg1.png")
game_sc.create_image(0, 0, anchor = NW, image = img_obj2)

grid = [game_sc.create_rectangle(x * TILE, y * TILE, x * TILE+TILE, y * TILE+TILE) for x in range(20) for y in range(23)]
...
sc.create_text(775, 580,text="record:", font=("Times New Roman", 30),fill="#d9005a", anchor=NW)
_record = sc.create_text(775, 620,text=record, font=("Times New Roman", 30),fill="#d9005a", anchor=NW)

sc.create_text(775, 710,text="score:", font=("Times New Roman", 30),fill="#d9005a",  anchor=NW)
_score = sc.create_text(775, 750,text=str(score), font=("Times New Roman", 30),fill="#d9005a", anchor=NW)
...
while app_running:
    if app_running:
        ...
        # draw next figure
        for i in range(4):
            figure_rect_x = next_figure[i][0] * TILE + 450
            figure_rect_y = next_figure[i][1] * TILE + 385
            fig2.append(sc.create_rectangle(figure_rect_x, figure_rect_y, figure_rect_x + TILE,
                                figure_rect_y + TILE, fill=rgb_to_hex(next_color)))
        # draw titles
